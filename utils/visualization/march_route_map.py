...
def create_march_route_map(gps_data: pd.DataFrame, participant_name: str = "Participant") -> go.Figure:
    """
...
def create_multi_participant_route_map(all_gps_data: pd.DataFrame) -> go.Figure:
    ...
    for idx, user_id in enumerate(participants):
        participant_data = all_gps_data[all_gps_data['user_id'] == user_id].sort_values('timestamp_minutes')
    ...
    fig.update_layout(
        title=dict(
            text="March Routes - All Participants",
            x=0.5,
            xanchor='center'
        ),
        mapbox=dict(
...
def create_elevation_profile(gps_data: pd.DataFrame, participant_name: str = "Participant") -> go.Figure:
    """
    ...
    Returns:
        Plotly Figure object with elevation profile
    """
    ...
    if 'cumulative_distance_km' in elevation_data.columns and elevation_data['cumulative_distance_km'].notna().any():
    ...
    stats_text = (
        f"Max: {max_elev:.0f}m | Min: {min_elev:.0f}m | "
        f"Ascent: {total_ascent:.0f}m | Descent: {total_descent:.0f}m"
    )
