...
# Import components
# Import callbacks
from src.app.callbacks.navigation_callbacks import register_navigation_callbacks
from src.app.components.auth import create_login_form, create_user_info_dropdown

# Show participant detail view
from src.app.components.march.participant_detail import (
...
from src.database.utils import get_user_by_id, init_database_manager
...
def create_authenticated_layout(march_id=None, participant_id=None):
    ...
    if participant_id and march_id:
        if not user_can_view_participant(current_user.id, participant_id, current_user.role):
            from src.app.components.auth import create_access_denied
