...
Process temperature data from participant parquet files to compute core temperature
metrics for march events.
"""
...
class TemperatureProcessor:
    """Process temperature data to compute temps for multiple participants"""
    ...
    def __init__(
        ...
        march_id : int
            March event ID
        march_start_time : datetime, optional
            March start time for timestamp alignment
        gps_crossing_times : dict, optional
            GPS crossing times for data trimming
        """
    ...
    def process_participant(self, participant_id: str, temp_file: Path) -> pd.DataFrame | None:
        ...
        try:
            ...
            if not all(col in df_temp.columns for col in required_cols):
                logger.error(
                    f"Missing required columns in {temp_file}. Required: {required_cols}"
                )
                return None
        ...
        for participant_id, temp_files in participant_files.items():
            ...
            if participant_dfs:
                if len(participant_dfs) > 1:
                    logger.info(
                        f"Combining {len(participant_dfs)} date files for {participant_id}"
                    )
...
def main():
    """Main entry point for the temperature processor CLI"""
    parser = argparse.ArgumentParser(
        description="Process temperature data to compute temps for march participants",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process temperature data
  fitonduty-process-temp --data-dir /path/to/participants --march-id 1 --output ./output

  # With march start time for timestamp alignment
  fitonduty-process-temp --data-dir /path/to/participants --march-id 1 \\
    --march-start-time 2025-03-15T08:00:00 --output ./output

  # With GPS crossing times for trimming
  fitonduty-process-temp --data-dir /path/to/participants --march-id 1 \\
    --gps-trim-file gps_times.json --output ./output
        """,
    )
    ...
    parser.add_argument(
        "--output", default="./data/output", help="Output directory for CSV files"
    )
