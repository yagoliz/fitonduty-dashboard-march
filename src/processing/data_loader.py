        ...
        if custom_mapping:
            ...
            for participant_id, username in custom_mapping.items():
                if username in users:
                    ...
                    print(f"  ✓ Mapped {participant_id} -> {username} (user_id={users[username]})")
                else:
                    print(f"  ⚠️  Username '{username}' not found for participant {participant_id}")
            return mapping
...
def map_participant_ids(df, user_mapping):
    ...
    if unmapped_count > 0:
        ...
        print(f"  ⚠️  Could not map {unmapped_count} rows with IDs: {unmapped_ids}")
        df = df.dropna(subset=['user_id'])

    # Convert to integer
    df['user_id'] = df['user_id'].astype(int)

    print(f"  ✓ Mapped {len(df)}/{original_count} rows to database user IDs")
    return df
...
def load_march_health_metrics(conn, df, march_id):
    ...
    for _, row in df.iterrows():
        try:
            ...
            print(f"  ❌ Error loading metrics for user {row['user_id']}: {e}")

    print(f"  ✓ Loaded {loaded_count} health metrics records")
    return loaded_count
...
def load_march_hr_zones(conn, df, march_id):
    ...
    for _, row in df.iterrows():
        try:
            ...
            print(f"  ❌ Error loading HR zones for user {row['user_id']}: {e}")

    print(f"  ✓ Loaded {loaded_count} HR zones records")
    return loaded_count
...
def load_march_gps_positions(conn, df, march_id):
    ...
    print(f"  ✓ Loaded {loaded_count} GPS position records")
    return loaded_count
...
def main():
    ...
    try:
        with engine.begin() as conn:
            # Load in order: metrics -> hr_zones -> timeseries -> gps -> core_temp
            total_loaded = 0
